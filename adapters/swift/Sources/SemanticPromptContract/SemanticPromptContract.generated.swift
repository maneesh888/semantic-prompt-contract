...
public enum SemanticPromptContract {
    ...
    case "fix_grammar":
        ...
        return renderWriting(operationID: "fix_grammar", wireOperationID: "fix_grammar", input: input, parameters: validatedParameters, rules: [
            "Inspect the source from left to right and identify only definite local errors: misspellings, capitalization mistakes, punctuation mistakes, missing or extra short function words, and unambiguous agreement or word-form errors.",
            "This is an edit-card task, not a rewrite task. Do not make the text clearer, more formal, friendlier, more concise, or otherwise different in style. Do not replace a valid word or phrase with a synonym; for example, changing \"reply\" to \"respond\" is forbidden.",
            "Return one atomic correction result per distinct error. Repeated occurrences are separate errors. Never combine separate errors or return a sentence-level correction item.",
            "Set every issue item's type to exactly \"correction\".",
            "Copy original exactly from the source and make replacement the smallest text needed for that error. Prefer one word; use a span of two or three words only when a missing or extra word, agreement, or punctuation boundary cannot be represented with one word. A result item containing the full input or a full corrected sentence is invalid; the full corrected sentence belongs only in corrected_text.",
            ...
            "Use a specific mechanical-error title such as Capitalization, Subject-verb agreement, Article, Spelling, Missing word, Extra word, Word form, or Punctuation. Never use Word choice for a stylistic preference.",
            ...
            "Before returning an item, verify that original occurs exactly in the source, the change fixes a definite error, and the replacement makes no unrelated change. If uncertain, omit the item.",
            "Set corrected_text to the complete source with only the returned atomic corrections applied. If the input has no definite errors, return an empty results array, keep corrected_text equal to the input, and never invent a correction."
        ], maxTokens: 5000)
    case "rewrite":
