...
`semantic-prompt-contract` is the canonical, versioned source for reusable semantic instructions used by OpenKeyboard and its gateway diagnostics. Version `4.0.0` makes `rewrite`, `rewrite_core`, every `rewrite_*` style, and `improve` return one complete plain-text replacement. Grammar remains plain text; Translate, Summarize, Continue Writing, and keyboard suggestions retain their existing semantics.
