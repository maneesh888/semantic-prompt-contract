...
`semantic-prompt-contract` is the canonical, versioned source for reusable semantic instructions used by OpenKeyboard and its gateway diagnostics. Version `3.2.0` adds a contract-owned fast, single-error grammar diagnostic preset while retaining the `3.0.0` production renderings and the `3.1.0` translation preset.
